...
contract FlashLoanIntegrationLendingTest is Test, IDahliaFlashLoanCallback {
    ...
    function test_int_flashLoan_BadMarket(IDahlia.MarketId id) public {
        vm.assume(!vm.marketsEq($.marketId, id));
        vm.expectRevert(abi.encodeWithSelector(Errors.WrongStatus.selector, IDahlia.MarketStatus.Uninitialized));
        $.dahlia.flashLoan(id, 1, abi.encode(this.test_int_flashLoan_ZeroAssets.selector));
    }
